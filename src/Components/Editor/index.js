...
class Editor extends Component {
  constructor(props) {
    ...
    this.cube3DRef = React.createRef(); // Créez une référence pour le composant Cube3D
  }
  componentDidMount() {
    ...
    canvas.on("object:modified", this.handleCanvasChange);

    // Écoutez les événements de modification du canvas
    canvas.on("object:added", this.handleCanvasChange);
    canvas.on("object:removed", this.handleCanvasChange);
    canvas.on("text:changed", this.handleCanvasChange);
    canvas.on("mouse:up", this.handleCanvasChange); // Lorsque la souris est relâchée
    canvas.on("mouse:down", this.handleCanvasChange); // Lorsque la souris est enfoncée
    canvas.on("mouse:move", this.handleCanvasChange); // Lorsque la souris bouge
    canvas.on("mouse:wheel", this.handleCanvasChange); // Lorsque la molette de la souris est utilisée
    canvas.on("selection:created", this.handleCanvasChange); // Lorsqu'une sélection est créée
    canvas.on("selection:updated", this.handleCanvasChange); // Lorsque la sélection est mise à jour
    canvas.on("selection:cleared", this.handleCanvasChange);
    // Écoutez les événements spécifiques de déplacement et de redimensionnement du texte
    canvas.on("object:scaled", this.handleCanvasChange); // Redimensionnement
    canvas.on("object:moving", this.handleCanvasChange); // Déplacement
  ...
  handleCanvasChange = () => {
    ...
    // Obtenez la nouvelle image du canvas
  ...
  render() {
    ...
    return (
      <div id="Canvas">
        ...
        <Cube3D ref={this.cube3DRef} canvasDataURL={this.state.href} />
        {/* Afficher l'image mise à jour automatiquement */}
        {this.state.href && (
          <div>
            <h2>Image mise à jour :</h2>
            <img src={this.state.href} alt="Image mise à jour" />
          </div>
        )}
      </div>
    );
